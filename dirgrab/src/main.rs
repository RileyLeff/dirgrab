...
fn main() -> Result<()> {
    ...
    // Determine PDF Conversion (based on absence of --no-pdf)
    let convert_pdf = !cli.no_pdf; // Set based on the new flag
    if convert_pdf {
        info!("PDF text extraction will be attempted (default).");
    } else {
        info!("PDF text extraction is disabled (--no-pdf specified).");
    }
    ...
    let config = GrabConfig {
        ...
        no_git: cli.no_git,
        include_tree,
        convert_pdf, // Pass the calculated value
    };
